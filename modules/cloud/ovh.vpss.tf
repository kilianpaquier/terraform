...
resource "ovh_vps" "coolify" {
  count = contains(keys(data.cloudinit_config.coolify), "ovh") ? 1 : 0
  ...
  display_name   = "coolify.vps.ovh.net"
...
data "ovh_vps" "coolify" {
  depends_on   = [ovh_vps.coolify]
  count        = length(ovh_vps.coolify)
  service_name = ovh_vps.coolify[0].service_name
}
