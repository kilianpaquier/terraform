...
# see https://github.com/integrations/terraform-provider-github/pull/2500
data "external" "environment_public_key" {
  for_each = toset([
  ...
  working_dir = "${path.module}/environment_public_key"

  query = {
    environment           = each.value
    owner                 = var.owner
    repository            = var.repository
    github_token_env_name = "TF_VAR_terraform_token"
  }
}
    ...
    merge([
      for env in var.environments : {
        for secret in env.secrets : "${env.environment}:${secret.secret_name}" => {
          sodium_public_key  = data.external.environment_public_key[env.environment].result.key
